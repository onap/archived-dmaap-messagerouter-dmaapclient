...
public class MRSimplerBatchPublisher extends MRBaseClient implements MRBatchingPublisher {
    ...
    private synchronized boolean sendBatch() {
        ...
        final String httpurl = MRConstants.makeUrl(host, fTopic, props.getProperty(Prop.PROTOCOL),
                props.getProperty(Prop.PARTITION));

        try {

            final ByteArrayOutputStream baseStream = new ByteArrayOutputStream();
            OutputStream os = baseStream;
            final String contentType = props.getProperty(Prop.CONTENT_TYPE);
    ...
    public synchronized MRPublisherResponse sendBatchWithResponse() {
        ...
        try (ByteArrayOutputStream baseStream = new ByteArrayOutputStream()) {
            os = baseStream;
            final String propsContentType = props.getProperty(Prop.CONTENT_TYPE);
    ...
    private void configureDME2() throws Exception {
        try {
            ...
            latitude = props.getProperty(Prop.LATITUDE);
            longitude = props.getProperty(Prop.LONGITUDE);
            version = props.getProperty(Prop.VERSION);
            serviceName = props.getProperty(Prop.SERVICE_NAME);
            env = props.getProperty(Prop.ENVIRONMENT);
            partner = props.getProperty(Prop.PARTNER);
            routeOffer = props.getProperty(Prop.ROUTE_OFFER);
            subContextPath = props.getProperty(Prop.SUB_CONTEXT_PATH) + fTopic;

            protocol = props.getProperty(Prop.PROTOCOL);
            methodType = props.getProperty(Prop.METHOD_TYPE);
            dmeuser = props.getProperty(Prop.USERNAME);
            dmepassword = props.getProperty(Prop.PASSWORD);
            contentType = props.getProperty(Prop.CONTENT_TYPE);
            handlers = props.getProperty(Prop.SESSION_STICKINESS_REQUIRED);

            MRSimplerBatchPublisher.routerFilePath = props.getProperty(Prop.DME2PREFERRED_ROUTER_FILE_PATH);
            ...
            String partitionKey = props.getProperty(Prop.PARTITION);
            ...
            if ("yes".equalsIgnoreCase(handlers)) {
                ...
                sender.addHeader("AFT_DME2_EXCHANGE_REPLY_HANDLERS",
                        props.getProperty(Prop.AFT_DME2_EXCHANGE_REPLY_HANDLERS));
