...
public class MRSimplerBatchPublisher extends MRBaseClient implements MRBatchingPublisher {
	...
	public void close() {
		try {
			...
			if (remains.isEmpty()) {
				getLog().warn(String.format("Closing publisher with %d messages unsent. Consider using MRBatchingPublisher.close( long timeout, TimeUnit timeoutUnits ) to recapture unsent messages on close.",
					remains.size()));
			}
	...
	private synchronized boolean sendBatch() {
		...
		try {
            ...
            if (ProtocolTypeConstants.AUTH_KEY.getValue().equalsIgnoreCase(protocolFlag)) {
                getLog().info(String.format("sending %d msgs to %s. Oldest: %d ms", fPending.size(), httpurl,
					nowMs - fPending.peek().timestamp));
