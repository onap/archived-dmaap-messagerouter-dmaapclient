    ...
    protected static MRBatchingPublisher createInternalBatchingPublisher(Properties props, boolean withResponse)
        ...
        if (withResponse) {
            pub = new MRSimplerBatchPublisher.Builder()
                    .againstUrlsOrServiceName(MRConsumerImpl.stringToList(props.getProperty(Prop.HOST)), MRConsumerImpl.stringToList(props.getProperty(Prop.SERVICE_NAME)), props.getProperty(Prop.TRANSPORT_TYPE))
                    .onTopic(props.getProperty(Prop.TOPIC))
                    .batchTo(Integer.parseInt(props.getProperty(Prop.MAX_BATCH_SIZE)),
                            Integer.parseInt(props.getProperty(Prop.MAX_AGE_MS).toString()))
                    .compress(Boolean.parseBoolean(props.getProperty(Prop.COMPRESS)))
        ...
        if (props.getProperty(Prop.TRANSPORT_TYPE).equalsIgnoreCase(ProtocolTypeConstants.AUTH_KEY.getValue())) {
            ...
            pub.setUsername(props.getProperty(Prop.USERNAME));
            pub.setPassword(props.getProperty(Prop.PASSWORD));
        } else {
            pub.setUsername(props.getProperty(Prop.USERNAME));
            pub.setPassword(props.getProperty(Prop.PASSWORD));
        }
    ...
    public static MRConsumer createConsumer(Properties props) throws FileNotFoundException, IOException {
        ...
        if (props.getProperty(Prop.TRANSPORT_TYPE).equalsIgnoreCase(ProtocolTypeConstants.AUTH_KEY.getValue())) {
            sub = new MRConsumerImpl.MRConsumerImplBuilder()
                    ...
                    .setConsumerId(props.getProperty(Prop.ID)).setTimeoutMs(timeout).setLimit(limit)
                    .setFilter(props.getProperty(Prop.FILTER))
                    .setApiKey_username(props.getProperty(Prop.AUTH_KEY))
                    .setApiSecret_password(props.getProperty(Prop.AUTH_DATE)).createMRConsumerImpl();
            sub.setAuthKey(props.getProperty(Prop.AUTH_KEY));
            sub.setAuthDate(props.getProperty(Prop.AUTH_DATE));
            sub.setUsername(props.getProperty(Prop.USERNAME));
            sub.setPassword(props.getProperty(Prop.PASSWORD));
        } else {
            sub = new MRConsumerImpl.MRConsumerImplBuilder()
                    .setHostPart(MRConsumerImpl.stringToList(props.getProperty(Prop.HOST)))
                    .setTopic(props.getProperty(Prop.TOPIC)).setConsumerGroup(group)
                    .setConsumerId(props.getProperty(Prop.ID)).setTimeoutMs(timeout).setLimit(limit)
                    .setFilter(props.getProperty(Prop.FILTER))
                    .setApiKey_username(props.getProperty(Prop.USERNAME))
                    .setApiSecret_password(props.getProperty(Prop.PASSWORD)).createMRConsumerImpl();
            sub.setUsername(props.getProperty(Prop.USERNAME));
            sub.setPassword(props.getProperty(Prop.PASSWORD));
        }
