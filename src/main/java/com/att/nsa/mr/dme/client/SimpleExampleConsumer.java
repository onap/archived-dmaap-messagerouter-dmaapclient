	...
	public static void main(String[] args) {
		...
		try {
			...
			while (true) {
				...
				for (MultivaluedMap.Entry<String,Object> entry: headersMap.entrySet()) {
                                    key = entry.getKey();
                                    logger.log("Header Key " + key);
                                    logger.log("Header Value " + headersMap.get(key));
				}
				// Header for HTTP Call.
				
				 Map<String, String> dme2headersMap=MRClientFactory.DME2HeadersMap;
                                 for(Map.Entry<String,Object> entry: dme2headersMap.entrySet()) {
                                     key = entry.getKey();
                                     logger.log("Header Key " + key);
                                     logger.log("Header Value " + dme2headersMap.get(key));
                                 }
